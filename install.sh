...
golangTools() {
	...
	cp "$HOME"/tools/Gf-Patterns/*.json ~/.gf
	echo -e "[$GREEN+$RESET] Done."
	...
	cd ~/tools
	git clone https://github.com/projectdiscovery/nuclei.git; cd nuclei/cmd/nuclei/; go build; mv nuclei /usr/local/bin/
	...
	echo -e "[$GREEN+$RESET] Installing gowitness"
...
additionalTools() {
	...
	echo -e "[$GREEN+$RESET] Installing Git-hound.."
	cd "$HOME"/tools/ || return
